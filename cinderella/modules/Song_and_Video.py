...
async def download_video(v_url):
    ...
    if not sender.id == me.id:
        rkp = await lazy.reply("`Let me check🔭`")
    else:
    	rkp = await lazy.edit("`Let me check🔭`")   
    ...
    type = "audio"
    await rkp.edit("`got it let me download`")
    if type == "audio":
        opts = {
            'format':
            'bestaudio',
        ...
        song = True    
    try:
        await rkp.edit("`hacking YouTube db..`")
    ...
    if song:
        ...
        await v_url.client.send_file(
            ...
            progress_callback=lambda d, t: asyncio.get_event_loop(
                ...
                progress(d, t, v_url, c_time, "Deleting 🃏",
                         f"{rip_data['title']}.mp3")))
        os.remove(f"{rip_data['id']}.mp3")
    elif video:
        await rkp.edit(f"`Preparing to Delet song :`\
        \n**{rip_data['title']}**\
        \nby *{rip_data['uploader']}*")
        await v_url.client.send_file(
            v_url.chat_id,
            ...
            progress_callback=lambda d, t: asyncio.get_event_loop(
                ...
                progress(d, t, v_url, c_time, "Hacking",
...
async def download_video(v_url):  
    lazy = v_url ; sender = await lazy.get_sender() ; me = await lazy.client.get_me()
    if not sender.id == me.id:
        rkp = await lazy.reply("`Let me hack`")
    else:
    	rkp = await lazy.edit("`hacking ytb`")   
    url = v_url.pattern_match.group(1)
    if not url:
         return await rkp.edit("`Error \nusage song <song name>`")
    search = SearchVideos(url, offset = 1, mode = "json", max_results = 1)
    ...
    try:
       url = q[0]['link']
    	...
    	return await rkp.edit("`failed to find`")
    type = "audio"
    await rkp.edit("`hacking YouTube..`")
    if type == "audio":
        opts = {
            'format':
            'best',
        ...
        video = True
    try:
        await rkp.edit("`cloning YouTube database`")
        with YoutubeDL(opts) as rip:
            rip_data = rip.extract_info(url)
    except DownloadError as DE:
        await rkp.edit(f"`{str(DE)}`")
